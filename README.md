...
这是本项目的核心。抓包分析后发现，不同站点的签到页都会在 HTML 里内嵌**机器可读**的签到台账，
只是格式不同。解析器会**自动识别**两种格式，并统一给出日期级别的确定性判断。
...
1. 页面自报的 `nowDate`（HHClub 提供）
2. 账号时区的今天（HDFans 等不提供的站点）
3. 台账中最大的日期 —— **仅在拿不到前两者时才用**

第 3 条绝不能优先：若今天尚未签到，台账最大日期会是"上次签到的日期"，
于是"今天"指向过去、`signed_today` 恒为真，**调度器将永远不会签到**。
...
粘贴到界面的「① 粘贴 cURL」框，点「解析并填入」，**站点、站点地址、Cookie、User-Agent
会自动识别**（例如粘贴 hdfans.org 的内容会自动选中 HDFans 并显示其积分单位）。
...
# 或手动指定（--site 可省略，会按地址自动识别）
python3 run.py add --name 'HDFans 主号' \
...
| `feishu` | `url`（自定义机器人 Webhook）、可选 `secret`、可选 `card` |
| `feishu_app` | `app_id`、`app_secret`、`receive_id`、可选 `receive_id_type`、可选 `card` |

可分别控制「成功 / 已签到 / 失败 / 恢复」是否通知；失败通知只在首次失败和最后一次重试时发送，避免刷屏。

#### 接入飞书

**方式一：自定义机器人（最简单，推荐个人使用）**

1. 打开飞书群 → 右上角 **设置** → **群机器人** → **添加机器人** → **自定义机器人**
2. 起个名字（例如 `PT签到`），复制生成的 **Webhook 地址**，形如
   `https://open.feishu.cn/open-apis/bot/v2/hook/xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx`
3. 安全设置任选其一（建议至少开一项）：
   - **签名校验** → 勾选后把 **Secret** 填入本系统的 `secret` 字段（本系统已实现加签）
   - **自定义关键词** → 填 `PT签到`（本系统通知标题固定以 `[PT签到]` 开头，天然满足）
   - **IP 白名单** → 填本机公网出口 IP
4. 在本系统「设置 → 通知」选择 **飞书（自定义机器人）**，填入 Webhook 地址
   （启用了签名校验再填 Secret），建议勾选 **使用消息卡片**（排版更好看）
5. 点「发送测试通知」验证

**方式二：应用机器人（发到指定群/用户，或需要更细的控制）**

1. 在[飞书开放平台](https://open.feishu.cn/app)创建企业自建应用，获取 **App ID** 与 **App Secret**
2. 开通 `im:message` 权限并发布应用
3. 把应用加入目标群，或拿到目标 `open_id` / 群 `chat_id`（`oc_` 开头）
4. 在本系统选择 **飞书（应用机器人）**，填 `app_id`、`app_secret`、`receive_id`；
   `receive_id_type` 保持 `chat_id` 即可（也可填 `open_id` / `email`）

**两个容易踩的坑（本系统已处理）**

- **飞书业务失败时仍返回 HTTP 200**（例如签名错误 `code=19021`、关键词不匹配），
  只看 HTTP 状态码会把失败当成成功。本系统会解析响应体中的 `code`，非 0 一律判为发送失败并提示原因。
- **加签算法容易写反**：正确做法是把 `"{timestamp}\n{secret}"` 当作 HMAC-SHA256 的 **key**、
  消息体留空，再做 Base64。本系统有固定测试向量防止实现漂移。

飞书纯文本消息不渲染 Markdown，因此文本模式会自动去掉 `**`；
勾选「使用消息卡片」则改用 `lark_md`，加粗与换行正常显示，并按结果着色
（成功绿、失败红、已签到灰、测试蓝）。
